    ...
    <div className="flex flex-col h-auto bg-[rgba(40,40,40,0.58)] mb-3 rounded-2xl shadow-sm border-gray-700 w-[48%] p-4">
      <div className="flex items-center mb-2">
        ...
        {organData.status !== "Unsure" &&
          "Test" // <Badge className={statusToColorMap[organData.status]}>{organData.status}</Badge>
        }
      </div>
      {organData.status === "Unsure" ? (
        ...
        <div className="mt-4">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {organData.tests.map((test, index) => (
              <span
                key={index}
                className="inline-block bg-blue-500 text-white text-xs font-bold py-1 px-2 rounded-full"
              >
                {test}
              </span>
            ))}
